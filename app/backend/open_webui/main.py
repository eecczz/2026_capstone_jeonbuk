...
from open_webui.utils.usage import get_user_id_from_request, record_usage_event
...
app.state.config.NEWS_API_URL = NEWS_API_URL_CONFIG
...
app.state.AIOHTTP_CLIENT_SESSION_SSL = AIOHTTP_CLIENT_SESSION_SSL
...
app.state.config.ENABLE_GRAPH_RAG_EXTRACTION = ENABLE_GRAPH_RAG_EXTRACTION
app.state.config.ENABLE_GRAPH_RAG_RETRIEVAL = ENABLE_GRAPH_RAG_RETRIEVAL
    ...
    if request.url.path.startswith("/api/v1"):
        ...
        if user_id:
            ...
            record_usage_event(
                ...
                input_bytes=request.headers.get("content-length"),
                output_bytes=response.headers.get("content-length"),
...
async def get_models(
    ...
    # Patch: inject suggestion_prompts from DB (lost during model merging)
    try:
        from open_webui.models.models import Models as _Models
        _sp_map = {}
        for _cm in _Models.get_all_models():
            if _cm.meta and hasattr(_cm.meta, "suggestion_prompts") and _cm.meta.suggestion_prompts:
                _sp_map[_cm.id] = _cm.meta.suggestion_prompts
        for model in models:
            _mid = model.get("id", "")
            if _mid in _sp_map:
                model.setdefault("info", {}).setdefault("meta", {})["suggestion_prompts"] = _sp_map[_mid]
    except Exception:
        pass
    log.debug(
...
async def chat_completion(
    ...
    try:
        ...
        # Auto-inject tool_ids from model meta if not provided by frontend
        _request_tool_ids = form_data.get("tool_ids", None)
        if not _request_tool_ids and model_info and model_info.meta:
            _model_meta = model_info.meta.model_dump() if hasattr(model_info.meta, 'model_dump') else (model_info.meta if isinstance(model_info.meta, dict) else {})
            _meta_tool_ids = _model_meta.get("toolIds", None)
            if _meta_tool_ids:
                _request_tool_ids = _meta_tool_ids
                form_data["tool_ids"] = _meta_tool_ids
                log.info(f"AUTO_INJECT_TOOLS: model={model_id}, tool_ids={_meta_tool_ids}")
    ...
    async def process_chat(request, form_data, user, metadata, model):
        try:
            ...
            # ── _skip_llm: 도구가 이미 응답을 생성한 경우 LLM 호출 건너뜀 ──
            if form_data.get("_skip_llm"):
                _tool_content = form_data.pop("_tool_content", "")
                form_data.pop("_skip_llm", None)
                log.info(f"SKIP_LLM: 도구 결과를 직접 스트리밍 ({len(_tool_content)} chars)")

                import uuid as _uuid

                async def _tool_stream():
                    _chunk = {
                        "id": f"chatcmpl-tool-{_uuid.uuid4().hex[:8]}",
                        "object": "chat.completion.chunk",
                        "model": form_data.get("model", ""),
                        "choices": [{
                            "index": 0,
                            "delta": {"role": "assistant", "content": _tool_content},
                            "finish_reason": None,
                        }],
                    }
                    yield f"data: {json.dumps(_chunk)}\n\n"
                    _done_chunk = {**_chunk, "choices": [{"index": 0, "delta": {}, "finish_reason": "stop"}]}
                    yield f"data: {json.dumps(_done_chunk)}\n\n"
                    yield "data: [DONE]\n\n"

                response = StreamingResponse(
                    _tool_stream(), media_type="text/event-stream"
                )
            else:
...
async def get_app_config(request: Request):
    ...
    return {
        **({"onboarding": True} if onboarding else {}),
        ...
        "version": VERSION,
        "news_api_url": app.state.config.NEWS_API_URL,
