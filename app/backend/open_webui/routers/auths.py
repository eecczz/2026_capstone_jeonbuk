...
async def update_password(
    ...
    if session_user:
        ...
        if user:
            ...
            # Clear must_change_pw flag after successful password change
            if result and session_user.info and isinstance(session_user.info, dict) and session_user.info.get("must_change_pw"):
                updated_info = {k: v for k, v in session_user.info.items() if k != "must_change_pw"}
                Users.update_user_by_id(session_user.id, {"info": updated_info or None}, db=db)
            return result
        else:
...
async def signin(
    ...
    # =========================
    # 1️⃣ SSO 로그인 (헤더가 실제로 있을 때만)
    # =========================
    if (
        WEBUI_AUTH_TRUSTED_EMAIL_HEADER
        and WEBUI_AUTH_TRUSTED_EMAIL_HEADER in request.headers
    ):
        email = request.headers.get(WEBUI_AUTH_TRUSTED_EMAIL_HEADER, "").strip().lower()
    ...
    # =========================
    # 2️⃣ 기존 OpenWebUI ID / PASSWORD 로그인
    # =========================
