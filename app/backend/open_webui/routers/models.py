...
async def _rebuild_models_cache(request: Request, user=None):
    """Rebuild Redis MODELS cache after GPTs create/update/delete."""
    try:
        from open_webui.utils.models import get_all_models
        await get_all_models(request, user=user)
    except Exception as e:
        log.debug(f"Failed to rebuild models cache: {e}")
...
async def get_models(
    ...
    for model in result.items:
        d = model.model_dump()
        # Strip heavy fields for list view (saves ~3.5MB per request)
        if d.get("meta"):
            d["meta"].pop("knowledge", None)
            img = d["meta"].get("profile_image_url") or ""
            if isinstance(img, str) and len(img) > 1000:
                d["meta"]["profile_image_url"] = "/static/favicon.png"
        if d.get("params"):
            d["params"].pop("system", None)
        items.append(ModelAccessResponse(
            **d,
            write_access=(
    ...
    return ModelAccessListResponse(items=items, total=result.total)
...
async def create_new_model(
    ...
    if not is_valid_model_id(form_data.id):
        ...
        if model:
            # Auto-grant public read access so GPTs are visible to all users
            AccessGrants.grant_access(
                resource_type="model",
            ...
            await _rebuild_models_cache(request, user=user)
            return model
        else:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
