...
class ModelsTable:
    ...
    def search_models(
        ...
        with get_db_context(db) as db:
            ...
            if filter:
                is_active = filter.get("is_active")
                if is_active is not None:
                    user_id_filter = filter.get("user_id")
                    if user_id_filter:
                        # Owner can see their own inactive models
                        query = query.filter(or_(Model.is_active == is_active, Model.user_id == user_id_filter))
                    else:
                ...
                if order_by == "name":
                    if direction == "asc":
                        query = query.order_by(Model.is_active.desc(), Model.name.asc())
                    else:
                        query = query.order_by(Model.is_active.desc(), Model.name.desc())
                elif order_by == "created_at":
                    if direction == "asc":
                        query = query.order_by(Model.is_active.desc(), Model.created_at.asc())
                    else:
                        query = query.order_by(Model.is_active.desc(), Model.created_at.desc())
                elif order_by == "updated_at":
                    if direction == "asc":
                        query = query.order_by(Model.is_active.desc(), Model.updated_at.asc())
                    else:
                        query = query.order_by(Model.is_active.desc(), Model.updated_at.desc())
