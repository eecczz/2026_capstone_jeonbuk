"""
Background task modules for Open WebUI.

이 패키지는 두 종류의 백그라운드 작업 로직을 담는다:

1) **Chat task 관리 (기존)** — 이 __init__.py 에 포함 (하위 호환성 유지).
   Redis 기반 task 추적, 생성/취소, 아이템별 태스크 관리. 원래 단일 파일
   `open_webui/tasks.py` 였으나, 크롤러 모듈 추가로 이 디렉토리를 패키지로
   전환하면서 기존 함수들을 여기로 이동. Python 은 `tasks.py` 와 `tasks/`
   가 동시에 있을 수 없으므로 (패키지 우선) 내용을 흡수해야 한다.

2) **크롤러 모듈** — `open_webui.tasks.crawler` / `open_webui.tasks.crawler_sites`.
   전북도청 + 직속기관 홈페이지 일별 배치 크롤러.

기존 코드의 `from open_webui.tasks import create_task, stop_item_tasks` 형태
import 를 그대로 쓸 수 있도록 아래 함수들을 이 __init__.py 에 정의한다.
"""
