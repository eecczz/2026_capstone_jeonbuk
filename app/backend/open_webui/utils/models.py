...
async def get_all_models(request, refresh: bool = False, user: UserModel = None):
    # Fast path: MODELS is shared via Redis across all workers.
    # If Redis has data but this worker's per-process BASE_MODELS is cold,
    # return Redis cache directly instead of expensive 15s re-processing.
    if (
        request.app.state.MODELS
        and not request.app.state.BASE_MODELS
        and isinstance(request.app.state.MODELS, RedisDict)
        and request.app.state.config.ENABLE_BASE_MODELS_CACHE
        and not refresh
    ):
        log.debug("get_all_models() fast path: returning from Redis cache (cold worker)")
        ...
        if custom_model.base_model_id is None:
            ...
            if model:
                if custom_model.is_active:
                    ...
                    # Override connection_type from custom model meta if set
                    try:
                        _meta = custom_model.meta.model_dump() if hasattr(custom_model.meta, 'model_dump') else (custom_model.meta or {})
                        _meta_ct = _meta.get("connection_type")
                        if _meta_ct:
                            model["connection_type"] = _meta_ct
                    except Exception:
                        pass
                else:
                    models.remove(model)

        elif (custom_model.is_active or (user and custom_model.user_id == user.id)):
            ...
            # Override: custom model's meta connection_type takes priority
            try:
                meta = custom_model.meta.model_dump() if hasattr(custom_model.meta, 'model_dump') else (custom_model.meta or {})
                meta_ct = meta.get("connection_type")
                if meta_ct:
                    connection_type = meta_ct
            except Exception:
                pass
