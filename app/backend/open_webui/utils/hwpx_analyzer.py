...
### Step 2.5: 각 chapter에 담길 내용 상상하기 ⭐ (핵심)

type을 고르기 전에, **각 chapter가 완성됐을 때 어떤 모습일지** 구체적으로 상상하세요:

**상상 항목**:
1. **구성 요소**: 요약 박스 / 섹션 헤더 / 세부 항목 / 참고 박스 / 각주 / 인용 / 수치 등 어떤 게 들어갈까?
2. **항목 개수**: 섹션 몇 개, 각 섹션 아래 세부 bullet 몇 개, 참고/각주 몇 개?
3. **계층 깊이**: flat (1단 목록)? 2단 (섹션+항목)? 3단 이상 (섹션+하위섹션+항목+세부)?
4. **문장 성격**: 짧은 제목 위주? 한 문장 설명 반복? 긴 서술?
5. **분량 감각**: 대략 몇 쪽 정도로 보일까?

### 예시 상상

- "뉴스 기사 - 정부 신규 정책 발표" chapter
  → 상상: 개요 박스 1개 + 정책 핵심 3-4개 bullet + 반응/논평 참고 박스 1-2개 + 간단 각주
  → 2단 깊이, 총 7-10개 항목 정도
  → **단순~중간 깊이 type** 선택

- "정책 평가 보고서" chapter
  → 상상: 검토배경 요약 + 평가 섹션 2-3개(각각 여러 세부 항목) + 통계 참고 박스 + 결론 박스
  → 3단 깊이, 총 20+ 개 항목
  → **깊은 type** 선택

### Step 3: 상상한 내용을 "가장 잘 담는" type 선택
