...
양식의 구조 분석 결과에서 **스타일 ID(paraPrIDRef, charPrIDRef)**를 가져와서, 소스 자료의 내용으로 문서를 새로 구성하는 JSON 명령을 생성합니다.

## 핵심 전략: 양식 본문을 지우고 소스 내용으로 재구성

양식과 소스 자료의 구조는 다를 수 있습니다. 양식의 슬롯에 내용을 끼워 맞추지 마세요.
대신:
1. 문서 헤더(제목, 날짜, 기관명)만 기존 슬롯에 교체
2. 본문은 `clear_body`로 제거
3. 소스 자료의 내용을 `add_paragraph`로 처음부터 구성 (양식의 스타일 ID를 재활용)

## 스타일 팔레트 (구조 분석에서 가져오세요)
구조 분석 결과의 paragraphs에서 각 level별 paraPrIDRef, charPrIDRef를 확인하세요. 이것이 "스타일 팔레트"입니다.
예시:
- level 2 (대분류 제목): paraPrIDRef="68", charPrIDRef="1" → □ 제목에 사용
- level 3 (소제목): paraPrIDRef="75", charPrIDRef="1" → ○ 항목에 사용
- level 4 (세부 내용): paraPrIDRef="78", charPrIDRef="1" → 본문에 사용
- level 5 (보충 설명): paraPrIDRef="13", charPrIDRef="13" → * 주석에 사용

실제 값은 구조 분석 결과에서 가져와 사용하세요.

## 명령 생성 순서

### Step 1: 헤더 교체
idx 0, 1, 2에 대해 set_paragraph_text로 소스 자료의 제목, 날짜, 기관명을 넣으세요.

### Step 2: 본문 제거
`clear_body`로 idx 3부터 끝까지 삭제하세요.
이렇게 하면 양식의 모든 본문(표, 텍스트 상자 포함)이 제거됩니다.

### Step 3: 소스 내용으로 문서 재구성
소스 자료(PDF)의 내용을 읽고, `add_paragraph`를 사용하여 문서를 새로 구성하세요.
각 문단에는 구조 분석에서 가져온 **스타일 ID(paraPrIDRef, charPrIDRef)**를 지정하세요.

소스 내용의 구조에 맞게 자유롭게 구성하세요:
- 대분류 제목 → level 2의 스타일 ID 사용
- 중분류 제목 → level 3의 스타일 ID 사용
- 세부 항목 → level 4~5의 스타일 ID 사용
- 보충 설명 → level 6~7의 스타일 ID 사용

**소스 자료의 모든 내용을 빠짐없이 반영하세요.**
**소스 자료에 없는 텍스트를 만들어내지 마세요.**

## 명령 타입

- **set_paragraph_text**: 기존 문단 텍스트 교체. {"type": "set_paragraph_text", "index": N, "text": "내용"}
...
```json
{
  "actions": [
    ...
    {"type": "set_paragraph_text", "index": 1, "text": "2005. 11. 21(월)"},
    {"type": "set_paragraph_text", "index": 2, "text": "대통령비서실"},
    {"type": "clear_body", "from_paragraph": 3},
    {"type": "add_paragraph", "paraPrIDRef": "68", "charPrIDRef": "1", "text": "□ 첫 번째 섹션 제목"},
    {"type": "add_paragraph", "paraPrIDRef": "75", "charPrIDRef": "1", "text": "ㅇ 세부 내용 1"},
    {"type": "add_paragraph", "paraPrIDRef": "75", "charPrIDRef": "1", "text": "ㅇ 세부 내용 2"}
  ]
}
```

## 중요
1. idx 0~2만 set_paragraph_text를 사용하세요 (제목, 날짜, 기관명)
2. **반드시 clear_body를 사용하여 idx 3부터 양식 본문을 제거하세요**
3. add_paragraph로 소스 내용을 전부 넣으세요. 내용을 생략하지 마세요
4. paraPrIDRef, charPrIDRef는 반드시 구조 분석 결과에서 가져온 값을 사용하세요
5. 소스 자료에 없는 텍스트를 만들어내거나, 양식의 샘플 텍스트를 복사하지 마세요
"""
