...
def build_section_polish_prompt(items_1st: list[dict], **fill_kwargs) -> list[dict]:
    ...
    if _headline_candidates:
        ...
        candidates_section += (
            ...
            "- **순서 강제**: (1) source 본문에서 의미 관계 (병렬 / 수단→결과 / 조치→목표 / 단계 진행 / 조건→결과 / 목적→행동 등) **먼저 추출** → (2) 추출한 관계를 `connector_limits` 의 connector 에 **최대한 매핑** (style_section 의 해당 role `relation_families_observed` 의 slot_template + applies_when / avoid_when 참고해 어떤 connector 가 어떤 관계 표현인지 판단).\n"
            "- **한 connector 만 반복 사용 X** — 양식이 사용한 다양한 connector 를 source 의미 관계에 따라 활용. 한 source 관계가 여러 connector 로 표현 가능해도 양식 sample 에서 더 정확한 connector (같은 의미 관계 표현 case 더 많은 것) 우선.\n"
            "- **`및` 양보 규칙**: source 가 단순 병렬로 보여 `및` 쓰고 싶어도, source 의미가 다른 관계 (목적→행동 / 수단→결과 등) 와 더 정확히 맞으면 `및` 양보하고 그 관계 connector 우선 선택.\n"
            "- **매핑 안 되는 source 관계는 표현 X** — 명사구 자체 줄여 핵심만 남김. 한 connector 한계 초과 시 다른 connector 로 치환 금지 (가짜 관계 만들기 X).\n"
    ...
    if _body_polish_candidates:
        ...
        candidates_section += (
            ...
            "- **순서 강제**: source 본문에서 의미 관계 먼저 추출 → connector_limits 의 connector 에 매핑 (style_section 의 relation_families_observed 참고).\n"
            "- 한 connector 만 반복 X — 양식이 사용한 다양한 connector 를 source 관계에 따라 활용. `및` 양보 가능 — source 가 다른 관계와 더 정확히 맞으면 그 관계 connector 우선.\n"
            "- 매핑 안 되는 source 관계는 표현 X — 명사구 자체 축소. 다른 connector 치환 X (가짜 관계 X).\n"
            "- 같은 connector 반복 ≤ `max_same_connector` 회 (보조 한계).\n"
