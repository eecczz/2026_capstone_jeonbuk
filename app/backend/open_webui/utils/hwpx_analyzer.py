...
양식의 구조(테두리 상자, 표, 배경색)를 **그대로 보존**하면서, 소스 자료의 내용으로 교체하는 JSON 명령을 생성합니다.

## 핵심 전략: 양식 구조 보존 + 내용만 교체

양식의 시각적 구조(박스, 표, 서식)는 절대 삭제하지 마세요.
대신 3가지 명령으로 내용을 교체합니다:

### 1. set_paragraph_text — 기존 문단의 텍스트 교체
양식의 기존 문단에 소스 내용을 넣습니다. 서식(글꼴, 크기, 들여쓰기)이 보존됩니다.
표(텍스트 상자) 안의 내용을 바꾸려면 set_cell을 사용하세요.

### 2. set_cell — 표 셀의 텍스트 교체
양식의 1x1 표(텍스트 상자)에 소스 내용을 넣습니다. 테두리, 배경색이 보존됩니다.

### 3. clone_paragraph — 슬롯이 부족하면 복제
양식에 □ 항목이 3개인데 소스에 5개가 필요하면, 기존 □ 항목을 2개 복제합니다.
복제하면 테두리, 표 구조가 그대로 유지됩니다.

### 4. remove_paragraph — 슬롯이 남으면 제거
양식에 과제가 9개인데 소스에 3개만 필요하면, 나머지 6개를 제거합니다.

## 매핑 순서

### Step 1: 헤더 교체
idx 0 (제목), idx 1 (날짜), idx 2 (기관명)에 소스 내용을 넣으세요.
표(table 0)에 제목이 있으면 set_cell도 사용하세요.

### Step 2: 본문 매핑
구조 분석의 각 paragraph idx에 대해:
- 소스에 대응 내용이 있으면 → set_paragraph_text로 교체
- 소스에 대응 내용이 없으면 → remove_paragraph로 제거

구조 분석의 각 table value_cell에 대해:
- 소스에 대응 내용이 있으면 → set_cell로 교체
- 없으면 → set_cell로 빈 문자열("") 설정

### Step 3: 부족한 슬롯 복제
소스 내용이 양식 슬롯보다 많으면, 같은 유형의 마지막 슬롯을 clone_paragraph로 복제하세요.
예: idx 7이 "ㅇ 세부항목"이고 더 필요하면 → clone_paragraph source=7

## 명령 타입

- **set_paragraph_text**: {"type": "set_paragraph_text", "index": N, "text": "내용"}
- **set_cell**: {"type": "set_cell", "table": N, "row": R, "col": C, "text": "내용"}
- **clone_paragraph**: {"type": "clone_paragraph", "source": N, "text": "내용"}
  원본 문단(표/박스 포함)을 복제하여 바로 뒤에 삽입, 텍스트만 교체
- **remove_paragraph**: {"type": "remove_paragraph", "index": N}
- **remove_table**: {"type": "remove_table", "table": N}
...
{
  "actions": [
    {"type": "set_paragraph_text", "index": 0, "text": "소스 제목"},
    {"type": "set_paragraph_text", "index": 1, "text": "2005. 11. 21"},
    {"type": "set_cell", "table": 0, "row": 1, "col": 0, "text": "소스 제목"},
    {"type": "set_paragraph_text", "index": 6, "text": "□ 소스 섹션 1"},
    {"type": "set_paragraph_text", "index": 7, "text": "ㅇ 소스 내용 1"},
    {"type": "clone_paragraph", "source": 7, "text": "ㅇ 소스 내용 2 (추가)"},
    {"type": "remove_paragraph", "index": 50},
    {"type": "remove_paragraph", "index": 49}
  ]
}
```

## 중요
1. **clear_body, add_paragraph를 사용하지 마세요** — 양식 구조가 파괴됩니다
2. 기존 슬롯에는 set_paragraph_text / set_cell로 내용을 교체하세요
3. 슬롯이 부족하면 clone_paragraph로 같은 유형의 슬롯을 복제하세요
4. 슬롯이 남으면 remove_paragraph로 제거하세요 (높은 index부터 제거)
5. 소스 자료에 없는 텍스트를 만들어내지 마세요
6. 소스 자료의 모든 내용을 빠짐없이 반영하세요
"""
