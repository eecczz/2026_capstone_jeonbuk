...
def build_adaptation_plan_prompt(
    ...
    user_msg = (
        ...
        "- source가 여러 영역(예: 클라우드 + AI + 운영 효율)을 다루면 그것들을 **모두 묶는 상위 개념**\n"
        "  (예: 스마트행정 종합)을 focus로. 한 영역만 선택하면 다른 영역이 본문에서 누락됩니다.\n"
        ...
        "  source가 명확히 서로 다른 정책 패키지를 다루면 (예: 정책품질관리 + 방문취업제 + 문서관리카드)\n"
        "  그 중 하나만 focus로 선택. 다만 같은 흐름의 여러 영역(클라우드+AI+운영)은 묶어서 처리.\n\n"
        "추가 가이드:\n"
        ...
        "  좁은 특정 분야(예: 클라우드 전환)로 한정하면 그 안에 안 들어가는 source 영역\n"
        "  (예: AI 도입, 정보시스템 운영)이 본문에서 누락됩니다.\n"
        "  source가 여러 영역(A+B+C)을 다루면 그것들을 모두 묶는 상위 명사(예: 스마트행정 종합)로.\n"
        "- source 도메인 명사가 양식 chapter title에 자연스럽게 맞도록 보정.\n"
        "- '같게 vs 다르게'를 먼저 분류하지 말고 자연스럽게 결정. 결과가 original과 같든 다르든 OK.\n"
        "- placeholder(○○○, [기관명], <연도>)가 있으면 source 도메인 명사로 채움.\n"
        "- 길이: original_title의 ±20% 정도 (양식 TOC/헤더 layout 보존). 부가 수식어 추가 X.\n"
        "- 길이 핑계로 source 도메인 보정 생략 금지. 길이 우선시한다고 결과가 부정확하면 안 됨.\n"
        ...
        "      자식 마커 (󰊱, 󰊲, ➊, ➋, □, ○ 등)는 from에 포함하지 마세요. 양식 마커는 보존됩니다.\n"
        "      예: TOC text='󰊱 민생경제 안정' → from='민생경제 안정' (마커 󰊱 제외).\n"
        "    - to: '?' 한 글자\n"
