...
### 대제목 타입 분석 (chapter_types)
양식의 level 1 대제목들을 분석하여, 하위 구조 패턴이 다른 **대제목 타입**을 식별하세요.

1. paragraphs에서 level 1인 문단(대제목)을 찾으세요
2. 각 대제목부터 다음 대제목 직전까지의 하위 role 시퀀스를 관찰하세요
3. **하위 구조가 동일한 대제목은 같은 타입**으로 묶으세요
4. 각 타입에 대해 하위 role의 **계층 + 반복/선택** 패턴을 기술하세요

#### 패턴 기술 규칙
- `repeat: true` — 같은 위치에서 여러 번 반복 가능 (양식에서 2번 이상 나온 role)
- `repeat: false` — 1번만 나오는 role
- `optional: true` — 없어도 되는 항목 (같은 타입의 다른 대제목에서는 빠진 role)
- `children` — 해당 role 바로 아래에 오는 하위 role들

#### 부모-자식 필수 관계
**pattern의 트리는 양식의 실제 포함 관계를 정확히 반영해야 합니다.**

- role B가 양식에서 항상 role A 아래에만 나오면 → B는 A의 children에 넣으세요
- role B가 양식에서 A 없이도 독립적으로 나오면 → B를 A의 children이 아닌 같은 level에 넣으세요

예시 1: ①이 항상 ➊ 아래에만 나오는 양식
```json
{
  "task_detail": {
    "repeat": true,
...
**양식 XML에서 실제로 어떤 role 아래에 어떤 role이 나오는지를 관찰하고, 그 관계를 그대로 트리로 옮기세요.**

#### 판단 예시
양식에서 대제목 아래 구조가:
```
section_header → detail_item, detail_item, note → section_header → detail_item
```
이면:
```json
{
  "section_header": {
    "repeat": true,
    "children": {
      "detail_item": { "repeat": true },
      "note": { "repeat": false, "optional": true }
    }
  }
}
```
...
```json
{
  ...
  "chapter_types": {
    "overview": {
      ...
      "description": "추진배경, 현황, 기대효과 등 개관 성격의 대분류",
      "pattern": {
        "section_header": {
          "repeat": true,
          "children": {
    ...
    "strategy": {
      "title_role": "chapter_title",
      "description": "추진전략, 세부과제 등 과제 중심 대분류",
      "pattern": {
        "section_header": {
          "repeat": true,
          "children": {
...
- **chapter_types의 타입 이름은 자유** — 양식의 성격에 맞게 지정 (overview, strategy, summary 등)
- **chapter_types에는 level 1 대제목의 하위 구조만 기술** — cover, toc, fixed는 제외
- **하위 구조가 동일한 대제목은 반드시 같은 타입으로 묶으세요**
"""
