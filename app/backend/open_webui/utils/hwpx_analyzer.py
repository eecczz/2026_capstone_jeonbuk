...
- description을 보고 판단: "문서 전체 제목" / "작성일자" / "목차" 등 표지 성격 → level 0
- description이 "장/대분류 제목"이면 level 1
- description이 "전략 제목", "과제 제목" 등이면 문맥상 level 결정 (보통 2~3)
- 마커 없고 description이 비슷하면 같은 level
...
같은 role 여부는 아래 기준의 조합으로 결정:

### 기준 1: marker 계열이 다르면 다른 role
- marker의 **유니코드 블록**이 다르면 다른 계열:
  - `➊ (U+278A, Dingbat)` vs `󰊱 (U+F02B1, PUA)` — 같은 "원번호"처럼 보여도 **다른 블록** → 다른 계열
  - `① (U+2460, Enclosed Alphanumeric)` vs `➊ (U+278A)` — 다른 블록 → 다른 계열
- marker의 **기본 문자 family**가 다르면 다른 계열:
  - `ㅇ` vs `*` vs `□` → 다른 계열
  - `1)` vs `가.` → 다른 계열

### 기준 2: 같은 marker 계열이라도 level이 다르면 다른 role
- 같은 `➊➋➌` 시퀀스라도 level 3에서 나타나는 것과 level 5에서 나타나는 것은 **다른 구조적 위치** → 다른 role
- 예: `➊` at level 3 → "strategy_numbered", `➊` at level 5 → "task_numbered"

### 기준 3: 같은 marker 계열 + 같은/비슷한 level = 같은 role
- **같은 계열 = 시퀀스 관계**:
  - `*, **, ***, ****` → 별표 반복 순번 → 하나의 role
  - `1), 2), 3), 4)` → 숫자+괄호 순번 → 하나의 role
  - `➊, ➋, ➌, ➍` → 네거 원문자 순번 → 하나의 role
  - `①, ②, ③, ④` → 원문자 순번 → 하나의 role
  - `가., 나., 다.` → 한글 순서 → 하나의 role
- 같은 계열의 변형은 합쳐야 함. 깊이·번호 접미사로 세분화 금지.

### 기준 4: marker 없는 문단은 description·level로 판단
- description이 유사하고 level도 같으면 같은 role
- "표지 제목", "작성일자", "목차" 등 구조적 위치가 고유하면 고유 role
- 같은 "섹션 제목 박스"가 여러 곳에 있으면 하나의 role
...
- **기준 1, 2 엄수**: marker 블록 다름 → 다른 role, level 다름 → 다른 role
- **기준 3 엄수**: 같은 시퀀스 계열은 묶기
