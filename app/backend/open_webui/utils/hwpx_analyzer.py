...
양식 XML을 분석하여 각 필드의 계층 구조, 번호 기호, 용도, **역할(role)**을 JSON으로 출력합니다.
...
- **role**: 이 문단의 서식 역할. **같은 서식(paraPrIDRef+charPrIDRef+표구조)을 공유하는 문단은 반드시 같은 role**
...
2. role 이름은 자유롭게 지정하되, 용도를 알 수 있게 하세요
   예: "chapter_title", "section_header_1x3", "subsection_box_1x1", "detail_item", "note", "summary_box", "spacer", "toc", "cover_title"
3. 특수 역할:
   - "spacer": 빈 문단 (구분용 빈 줄)
   - "toc": 목차 문단
...
3. **"(고정 텍스트, 수정 불필요)"는 극히 제한적으로만 사용하세요**
   오직 다음 경우만 해당: 페이지 번호 컨트롤, 머리글/바닥글, 구분선, "붙임" 같은 순수 레이아웃 요소
   **양식의 내용 텍스트(제목, 본문, 요약, 과제명 등)는 절대 "고정 텍스트"가 아닙니다** — 이것들은 새 내용으로 교체될 샘플입니다
   잘못된 예: "Ⅰ. 추진성과 및 평가 (고정 텍스트)" ← 이건 제목 슬롯이지 고정이 아님
...
- **value_cells는 반드시 [{"row": 0, "col": 0}]으로 설정하세요** — 빈 배열 []은 금지
- headers는 빈 배열 []
- 텍스트 상자 안의 내용도 교체 대상입니다 (고정 텍스트가 아님)
...
{
  "paragraphs": [
    {"idx": 0, "level": 0, "marker": "", "role": "cover_title", "description": "문서 전체 제목 (연도+기관+사업명+문서종류 형식)", "paraPrIDRef": "5", "charPrIDRef": "12"},
    {"idx": 1, "level": 0, "marker": "", "role": "cover_date", "description": "작성일자 (yyyy. m. d. 형식)", "paraPrIDRef": "3", "charPrIDRef": "8"},
    {"idx": 2, "level": 0, "marker": "", "role": "cover_org", "description": "발신 기관명", "paraPrIDRef": "3", "charPrIDRef": "8"},
    {"idx": 3, "level": 1, "marker": "Ⅰ.", "role": "chapter_title_box", "description": "대분류 제목 (텍스트 상자)", "paraPrIDRef": "3", "charPrIDRef": "8"},
    {"idx": 4, "level": 2, "marker": "□", "role": "subsection", "description": "중분류 항목 제목", "paraPrIDRef": "0", "charPrIDRef": "0"},
    {"idx": 5, "level": 3, "marker": "ㅇ", "role": "detail_item", "description": "세부 항목의 설명 본문", "paraPrIDRef": "0", "charPrIDRef": "0"},
    {"idx": 6, "level": 2, "marker": "□", "role": "subsection", "description": "중분류 항목 제목", "paraPrIDRef": "0", "charPrIDRef": "0"},
    {"idx": 7, "level": 3, "marker": "ㅇ", "role": "detail_item", "description": "세부 항목의 설명 본문", "paraPrIDRef": "0", "charPrIDRef": "0"}
  ],
  "tables": [
...
- 같은 구조적 패턴의 필드는 반드시 같은 description과 **같은 role**을 사용하세요
- **같은 서식(paraPrIDRef+charPrIDRef+표구조)이면 반드시 같은 role** — 이것이 가장 중요합니다
- 표의 headers(라벨)와 value_cells(데이터)를 정확히 구분하세요
- **1x1 표의 value_cells를 빈 배열로 두지 마세요** — 반드시 [{"row": 0, "col": 0}]
- **"(고정 텍스트, 수정 불필요)"는 레이아웃 요소에만 사용** — 내용 텍스트에 사용 금지
"""

CONTENT_MAPPING_PROMPT = """당신은 HWPX 문서 작성 전문가입니다.
양식의 구조 패턴을 보고, 소스 자료의 내용을 **role(역할) 기반**으로 태깅하여 JSON을 생성합니다.

## 핵심 전략: 양식의 계층 구조를 재현

양식의 **내용**(조달, 과제 등)은 무시하세요. 양식은 서식 틀일 뿐입니다.
중요한 것은 양식의 **구조 패턴** — role들 사이의 계층 관계입니다.

예를 들어 양식 구조가 이렇다면:
  [chapter_title_box] "Ⅰ. 추진성과"
    [summary_box] "◈ 공공조달을 통해..."
    [subsection_title] "□ 경제위기 극복"
      [detail_item] "ㅇ 레미콘 등..."
      [detail_item] "ㅇ 중소 조달기업..."
    [subsection_title] "□ 조달정책 방향"
      [detail_item] "ㅇ 규제혁신..."

→ 소스 내용도 **같은 계층 구조**로 태깅:
  [chapter_title_box] "1. 검토 배경"
    [subsection_title] "□ 정책품질관리제도 시행 현황"
      [detail_item] "ㅇ 05.7부터 전 부처 본격 실시"
      [detail_item] "ㅇ 491개 정책 42개 기관 관리 중"
    [subsection_title] "□ 점검 필요성"
      [detail_item] "ㅇ 지속 여부와 운영방안 점검"

## 마커 규칙
- 소스 원문에 있는 마커(◇, ◆, ■ 등)는 무시하세요
- 대신, 해당 role의 양식 마커를 사용하세요
  예: role이 "subsection_title"이고 양식 마커가 "□"이면 → "□ 내용"
...
```json
{
  ...
  "body": [
    {"role": "chapter_title_box", "text": "제목"},
    {"role": "subsection_title", "text": "□ 중분류"},
    {"role": "detail_item", "text": "ㅇ 세부 내용"},
    {"role": "detail_item", "text": "ㅇ 세부 내용"}
  ]
}
```
...
### header
- header의 key는 양식 구조의 role 이름 그대로 사용
- 문서 앞부분에 한 번만 나오는 요소: 표지 제목, 날짜, 기관명 등
- 소스에 해당 정보가 없으면 key를 생략

### body
- body 배열은 문서에 나타날 순서대로 나열
- **양식 구조 패턴의 계층 관계를 반드시 따르세요**:
...
- toc(목차) role이 양식에 있으면 **반드시 body에 포함** — 새 문서의 목차 내용으로 채우세요

### 항목 길이
- **각 항목의 길이는 양식 샘플과 비슷하게 유지하세요**
- 양식 샘플이 1~2문장이면 body 항목도 1~2문장
- 소스 내용이 샘플보다 길면 **같은 role로 여러 항목으로 나누세요**
- 여러 문단을 하나의 항목에 합치지 마세요

### 마커 규칙
- 소스 원문에 있는 마커(◇, ◆, ⇒, ※, O, □ 등)는 **무시하세요**
- 대신, 해당 role의 **양식 마커**를 사용하세요
  - 예: detail_item role의 양식 마커가 "ㅇ"이면 → "ㅇ 내용"
  - 예: section_header role의 양식 마커가 "□"이면 → "□ 내용"
- ※로 시작하는 보충 설명은 **note 계열 role로 분리**하세요 (note_sub_item 등)
- ※ 내용을 detail_item에 합치지 마세요

### role 다양하게 사용
- **양식에 있는 role을 최대한 다양하게 활용하세요**
- 소스 내용의 계층 구조에 맞는 role을 선택:
  - 대분류 제목 → chapter_title 계열
  - 중분류 제목 → section_header 계열
  - 세부 내용 → detail_item 계열
  - 보충/참고 → note 계열
  - 요약/강조 → summary/strategic 계열
- **양식 구조 패턴에서 사용된 role을 소스 내용에도 반드시 대응시키세요**
- 하나의 role만 반복하지 말고, 소스 내용의 성격에 맞는 role을 골라 사용

### 내용 개수
- 소스 내용이 양식보다 많아도 됩니다 — 시스템이 자동 복제
- 소스 내용이 양식보다 적어도 됩니다 — 시스템이 자동 제거

## 중요
1. 소스 자료에 없는 텍스트를 만들어내지 마세요
2. 소스 자료의 모든 내용을 빠짐없이 반영하세요
3. role은 반드시 양식 구조에 있는 role만 사용
4. **양식 구조 패턴의 계층 관계가 가장 중요합니다**
5. **각 항목의 길이는 양식 샘플과 비슷하게** — 긴 내용은 여러 항목으로 분리
6. **소스 원문의 마커는 무시하고 양식 마커를 사용**
7. **※ 보충 설명은 note 계열 role로 분리**
8. **양식 샘플의 문체와 말투를 따르세요** — 양식이 "~하고, ~하며, ~을 추진" 같은 행정체면 결과물도 동일한 문체로 작성
9. 반드시 JSON만 출력. 다른 설명 포함 금지
"""
