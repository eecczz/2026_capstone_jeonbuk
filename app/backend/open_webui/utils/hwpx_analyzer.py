...
level을 배정하고 나면 트리가 만들어집니다. 이 트리를 보고 **각 부모 role의 직계 자식 쌍에 대해 "한 번이라도 같은 부모 인스턴스 안에서 같이 나온 적이 있는가?"를 카운팅**하세요.

### 규칙 (기계적으로 적용 — 판단 X)

각 자식 쌍 (A, B)에 대해:
- 같은 부모 인스턴스 안에서 **단 한 번이라도 공존** → **배타 아님** (무조건)
- 같은 부모 인스턴스 안에서 **한 번도 공존하지 않음** → **배타** (무조건)

OX의 이분법입니다. "확실한가?"를 고민하지 마세요. 카운트 결과대로 따르면 됩니다.

### 절차

1. level 배정 후 각 부모 role을 훑는다
2. 부모 role의 **각 인스턴스**가 직계 자식으로 어떤 role들을 포함하는지 집합으로 기록
   - 예: `section_header` 인스턴스 6개가 각각 {ㅇ}, {ㅇ}, {ㅇ}, {ㅇ, *}, {➊, *}, {➊} 라면
3. 가능한 모든 자식 쌍을 나열하고 공존 횟수를 센다
   - (ㅇ, *): 1회
   - (➊, *): 1회
   - (ㅇ, ➊): **0회** ← 배타
4. 배타 쌍이 있으면 variant로 묶어 출력. 공존 그래프로 묶음:
   - ㅇ-*-➊: ㅇ과 ➊이 직접 엣지 없음 → 서로 다른 variant로 분리
   - variant A = {ㅇ, *}, variant B = {➊, *} (공통 * 는 양쪽 포함)
5. 배타 쌍이 하나도 없으면 그 부모는 스킵 (해당 부모는 exclusive_rules에 포함하지 않음)

### 중요 원칙

- **손자 이하는 무시** — 직계 자식만 보세요. level이 부모+1인 것만.
- **인스턴스 단위** — 같은 부모 role이라도 각 인스턴스마다 다른 자식을 가질 수 있음
- 부모 인스턴스가 **2개 이상**이어야 판단 가능 (1개뿐이면 배타 판단 불가 → 스킵)
- **의심이나 확신 여부 고민 금지** — 오로지 "공존 0회인가 1회 이상인가"만
...
### 필드 설명
- `parent`: 부모 role 이름
- `variants`: 공존 그래프의 maximal clique들. 각 variant는 **서로 공존해도 되는 자식들**의 집합
- `pairs_never_cooccurred`: 공존 0회 쌍 (검증용 — 이 쌍이 있다는 것 자체가 배타의 근거)

## 중요
- **모든 idx의 level을 출력하세요** — 하나도 빠뜨리지 마세요
- level은 정수. 최상위는 0
- `exclusive_rules`: 공존 0회 쌍이 발견된 **모든** 부모를 포함. 판단 여지 없음.
