...
1. **후보 5종류만 만든다**:
   ...
   **후보는 가장 짧은 의미 단위 명사구로 잡는다.** 한 후보 안에 `및 / 또는 / 그리고 / 하고 / 하며 / 통해 / 위한 / 등` 같은 연결 구조를 포함하지 않는다. 두 명사구가 연결어로 이어진 구간은 **두 개의 후보로 분리**한다.
...
- **한 paragraph 의 base 가 marker / 구두점만으로 채워지면 안 된다** — 본문 흐름 base 가 최소 1 개 이상 남아야 한다.
