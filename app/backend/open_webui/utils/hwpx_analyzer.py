...
**role은 출력하지 마세요.** role 분류는 level 판정 후 별도 단계(1c)에서 수행합니다.
...
## 중요
- **role도, level도 절대 출력하지 마세요** — 1b(level), 1c(role)에서 별도 결정합니다
...
LEVEL_ANALYSIS_PROMPT = """당신은 HWPX 양식의 **구조 분석** 전문가입니다 (1c / AI 2).
1b가 제공한 role 후보 + features를 받아 **전체 트리 구조**를 결정합니다.

## 역할 분담
- 1b / AI 1 (이전): semantic_role 후보 + 점수 (per-paragraph)
- **1c / AI 2 (이 단계)**: 전체 시퀀스 → level + parent + sibling + 후보 index 선택
...
1. **level**: 계층 깊이 (0=최상위)
2. **parent_idx**: 직접 부모 문단의 idx (없으면 null)
3. **sibling_group_id**: `children_of_<parent_idx>` (배타 variant 있으면 `_v1`, `_v2`)
4. **selected_role_candidate_index**: 1b 후보 중 어느 것 채택할지 (0 = 1순위)
   - 기본은 0 (1순위 채택)
...
### B. selected_role_candidate_index 선택

기본 0. 다음 경우 다른 index:
- 1순위 후보가 위치상 어색 → 2순위·3순위 중 마커 family·위치에 맞는 것 (`marker_family_fit`)
- 같은 부모 아래 형제들과 다른 종류 → 그 그룹에 맞는 후보 (`sibling_group_consistency`)
...
- `sibling_group_consistency`: 형제들과 같은 종류 맞춤
- `child_role_fit`: 부모-자식 관계에 더 맞춤
- `position_top_level`: 표지·대제목 등 최상위 위치 맞춤
- `other`: 기타 (간단한 부연을 description 필드에)

### C. parent_idx + sibling_group_id

- parent_idx: 직전 더 얕은 level 문단의 idx (스택 알고리즘으로 자연 도출)
- sibling_group_id: `children_of_<parent_idx>`. 한 부모 아래 명백한 variant 있으면 접미사
...
```json
{
  "paragraphs": [
    ...
    },
    {
      ...
      "level": 3,
      "parent_idx": 7,
      "sibling_group_id": "children_of_7",
      "selected_role_candidate_index": 0
    }
  ]
}
...
## 중요
- **모든 idx 출력**
- 4개 필드 필수: level, parent_idx, sibling_group_id, selected_role_candidate_index
...
def merge_levels_into_structure(
    ...
    # 1단계: decisions 적용 + selected_index → semantic_role 확정
    for p in structure.get("paragraphs", []):
        idx = p.get("idx", -1)
        ...
        if d:
            ...
            # selected_index로 semantic_role 확정
            ...
            if candidates:
                ...
                # AI 2가 옛 형식으로 final_role 줬으면 fallback
    ...
    # 2단계: structure_role 합성 (marker_family + semantic_role)
    for p in structure.get("paragraphs", []):
        sem_role = p.get("semantic_role") or p.get("role", "unknown")
        family = p.get("marker_family", "") or "no_marker"
        # family 정규화: char_X → X_marker, 그 외는 그대로
        if family.startswith("char_"):
            family_short = family[5:]  # 단일 char 마커 (□, ㅇ, *, ▪ 등)
            family_label = f"char{family_short}"  # 예: char□, charㅇ
        else:
            family_label = family
        structure_role = f"{family_label}__{sem_role}" if family != "no_marker" else sem_role
...
ROLE_CLASSIFICATION_PROMPT = """당신은 양식 문단의 **role 분석** 전문가입니다 (1b / AI 1).
각 문단을 독립적으로 보고 가능한 **semantic_role 후보들**을 점수화합니다.

## 역할 분담
- **1b / AI 1 (이 단계)**: semantic_role 후보 + 점수 (level·hierarchy 결정 안 함)
- 1c / AI 2 (다음 단계): 전체 시퀀스 + 후보 → level + parent + sibling + 후보 index 선택

⚠️ **반드시 후보를 다양하게 줘라**. 단일 후보 박지 마라. AI 2가 선택할 여지를 남겨야 한다.
...
- 예: marker_family가 "char_*" (별표)면 → `note`, `supplement_note`, `footnote` 등 보충 계열 1개는 들어가야

### 규칙 R4: marker_family 다른 항목을 같은 단일 후보로만 묶지 마라
- 같은 의미 후보 줘도 OK (▪과 ㅇ 둘 다 `bullet_item` 후보)
- 단, 서로 차이가 있을 가능성도 후보에 같이 두기 (예: `square_marker_item` vs `bullet_item`)
