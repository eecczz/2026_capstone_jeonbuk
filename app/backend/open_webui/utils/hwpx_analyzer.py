...
### 2단계: 순서 기반으로 매핑하세요
소스 자료의 구조와 양식의 구조가 다를 수 있습니다. 의미가 아닌 **순서**로 매핑하세요:
- 소스의 1번째 대섹션 → 양식의 1번째 level 1 슬롯
- 소스의 2번째 대섹션 → 양식의 2번째 level 1 슬롯
- 각 대섹션 내 소항목도 순서대로 하위 슬롯(level 2, 3)에 매핑

### 3단계: 초과분만 add_paragraph
1단계에서 모든 기존 슬롯을 채운 뒤에도 소스 내용이 남으면, 해당 섹션의 마지막 슬롯과 같은 서식(paraPrIDRef, charPrIDRef)으로 add_paragraph하세요.

### 양식보다 소스가 적으면
남는 슬롯은 빈 문자열("")로 비우거나 remove_paragraph로 제거하세요.
