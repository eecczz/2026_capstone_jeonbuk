...
**절대 금지**: 양식의 description 텍스트, 양식의 샘플 텍스트, 또는 스스로 만들어낸 텍스트를 넣지 마세요. 오직 소스 자료(PDF)에 있는 텍스트만 사용하세요.
...
### 대응 내용이 없으면 반드시 빈 문자열("")을 넣으세요
소스 자료에 대응하는 내용이 없는 필드는 빈 문자열("")을 넣으세요.
**같은 텍스트를 여러 슬롯에 반복해서 넣는 것은 금지입니다.** 내용이 부족하면 남는 슬롯은 전부 ""로 비우세요.
...
4. 양식보다 소스 내용이 적으면 남는 문단을 빈 문자열("")로 비우세요. **같은 텍스트를 반복하여 채우지 마세요**
5. 소스 자료의 내용을 빠짐없이 반영하되, 반드시 기존 슬롯을 먼저 활용하세요
6. **양식의 description이나 샘플 텍스트를 그대로 복사하여 넣지 마세요** — 반드시 소스 자료(PDF)의 텍스트만 사용하세요
"""
