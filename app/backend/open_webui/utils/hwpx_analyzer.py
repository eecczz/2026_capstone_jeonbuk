...
- **marker**: 번호 기호가 있으면 그대로 기록 (➊, 󰊲, Ⅰ., 1., 1), 가., 과제 1 등), 없으면 ""
  ...
  **마커 추출 엄수 사항**:
  ...
  - **일관성 유지**: 같은 role/섹션의 문단이라면 같은 **길이·구조**의 마커를 사용.
    옆 문단이 `"2)"`이면 이 문단도 `"1)"`이어야 함. 절대 `"1"`만 뽑지 마세요.
  - 애매하면 **양식의 다른 같은 패턴 문단 참고** — 주변에 같은 번호 체계가 있는지 확인.
...
def merge_levels_into_structure(
    ...
    Args:
        ...
        level_map: {idx: level}
        exclusive_rules: 1.5b AI가 출력한 형제 배타 규칙 리스트 (선택)
