...
## 핵심 전략: 양식의 구조적 역할을 소스 내용에 재활용

양식의 내용(조달, 과제 등)은 무시하세요. 양식은 **서식 틀**일 뿐입니다.
양식의 박스, 표, 문단을 소스 내용의 **같은 역할**에 재활용하세요:

| 양식의 구조 | → 소스에서의 용도 |
|---|---|
| "Ⅰ. 추진성과" 제목 박스 | → 소스의 1번째 대분류 제목 |
| "□ 경제위기 극복" 항목 | → 소스의 1번째 중분류 제목 |
| "ㅇ 주요 물자 안정" 항목 | → 소스의 1번째 세부 내용 |
| "과제 1 제목 박스" | → 소스의 1번째 섹션 제목 |
| "세부 과제 박스" | → 소스의 1번째 하위 항목 |

**의미가 아닌 구조적 역할(제목/항목/내용)로 매핑하세요.**
...
### Step 2: 본문 — 양식 슬롯을 순서대로 소스 내용으로 채우기
양식의 슬롯을 **위에서부터 순서대로** 소스 내용으로 교체하세요.
- 양식의 "대분류 제목" (Ⅰ., Ⅱ., Ⅲ.) → 소스의 대분류 제목
- 양식의 "□ 제목" → 소스의 중분류 제목
- 양식의 "ㅇ 내용" → 소스의 세부 내용
- 양식의 "과제 제목 박스" (set_cell) → 소스의 섹션 제목
- 양식의 "세부 과제 박스" (set_cell) → 소스의 하위 항목

### Step 3: 슬롯 개수 조절
- **소스가 더 많으면**: 같은 유형의 마지막 슬롯을 clone_paragraph로 복제
- **소스가 더 적으면**: 남는 슬롯을 remove_paragraph로 제거 (높은 index부터)
- **표가 남으면**: remove_table로 제거 (높은 번호부터)
