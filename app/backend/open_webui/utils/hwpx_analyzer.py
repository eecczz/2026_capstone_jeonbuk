...
### 규칙 R3.6: 같은 family 인스턴스는 같은 구조 역할이면 하나의 role로 통일

- 같은 marker_family에 속한 인스턴스들이 양식 트리에서 **같은 구조 위치/역할** (같은 위계, 같은 부모 그룹, 같은 자식 패턴)을 수행한다면, **모두 하나의 role 이름**으로 묶어라.
- 의미적 sub-genre (예: 같은 보충 항목 안에 "사례 vs 통계 vs 비교 vs 설명" 같은 micro 카테고리) 차이는 **role 이름이 아니라 description**에 남겨라.
- ❌ 같은 family 인스턴스마다 미세하게 다른 role 이름을 다는 행위 금지 (chapter_types signature 흩뿌림).
- 단, 같은 family 안에서 **진짜로 양식 트리에서 다른 노드 종류** (한 그룹은 자식 가지는 container, 다른 그룹은 leaf — 또는 한 그룹은 chapter root, 다른 그룹은 본문 항목 등) 이라면 그땐 분리 OK.
- 판단 기준: "이 둘의 차이가 양식 트리 그래프에서 다른 노드 종류로 구분되어야 하는가, 아니면 같은 노드 종류의 텍스트 내용 차이일 뿐인가?"
