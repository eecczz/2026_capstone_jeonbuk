...
- ⚠️ **무마커 제목 박스가 양식 안에서 여러 위계로 등장하면 후보를 위계별로 다양화**:
  - 가장 큰 묶음 제목: `broad_title` 또는 `chapter_header` 또는 `strategy_header`
  - 그 아래 묶음 제목: `section_title` 또는 `task_title`
  - 더 아래 묶음 제목: `subsection_title` 또는 `subtask_title`
  - 같은 description("제목"·"항목 제목")만으로 단일 후보 박지 마라. **인접 위계의 후보 1~2개 같이 줘서 1c가 위치로 고를 수 있게**.
  - 예: 어떤 무마커 박스에 1순위 `task_title`(0.72)을 줬으면 2순위로 `subtask_title`(0.60)나 `section_title`(0.55) 같이 둘 것.
