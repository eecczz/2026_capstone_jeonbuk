...
def build_adaptation_plan_prompt(
    ...
    user_msg = (
        ...
        "- **chapter title 매칭 (가장 중요)**:\n"
        "  - [chapters]의 모든 chapter에 대해 빠짐없이 매칭하세요 (chapter 1, 2, 3, ... 모두).\n"
        "  - **original_title이 TOC text에 정확히 같지 않을 수 있음** — original이 noisy해서 더 긴 경우 흔함\n"
        "    (예: original_title='추진성과 및 평가 - -', TOC text 안='추진성과 및 평가').\n"
        "    이 경우 **TOC text 안 깨끗한 chapter title 부분을 직접 찾아 from으로 사용**.\n"
        "    original_title을 그대로 from으로 쓰지 마세요 — TOC text에 없으면 매칭 실패.\n"
        "  - to는 adapted_title (마커 포함 여부는 from 형태와 맞춤).\n"
        "  - chapter마다 1개 replacement 필수. 누락 금지. (TOC 안에 진짜 없는 경우만 예외)\n"
        "- **자식 항목·페이지 번호 등 글자 있는 부분**: '?' 한 글자로 교체.\n"
        "  예: {'from': '민생경제 안정...', 'to': '?'}\n"
        "- **페이지 번호 처리 (강제)**: TOC text 안에 단독 숫자(예: ' 6 ', ' 10 ', '12')가 보이면\n"
        "  페이지 번호이므로 **반드시 '?'로 교체**하는 replacement 추가. 누락 금지.\n"
        "  예: {'from': ' 6 ', 'to': '?'}, {'from': '10', 'to': '?'}\n"
        "  앞뒤 공백 포함 substring 그대로 (TOC text 안에 존재하는 형태로).\n"
