...
양식의 구조 분석 결과를 보고, 소스 자료의 내용을 **role(역할) 기반**으로 태깅하여 JSON을 생성합니다.

## 핵심 전략: role 기반 매핑

양식의 내용(조달, 과제 등)은 무시하세요. 양식은 **서식 틀**일 뿐입니다.
각 문단에는 role이 부여되어 있습니다. 소스 내용을 적절한 role로 태깅하세요.

예를 들어 양식에 이런 role들이 있다면:
- "chapter_title_box": 대분류 제목 박스 (Ⅰ., Ⅱ.)
- "subsection": 중분류 항목 (□ 마커)
- "detail_item": 세부 내용 (ㅇ 마커)

소스 내용의 대분류는 chapter_title_box로, 중분류는 subsection으로, 세부내용은 detail_item으로 태깅합니다.
...
```json
{
  ...
  "body": [
    ...
    {"role": "summary_box", "text": "◈ 핵심 요약 내용"},
    {"role": "subsection", "text": "□ 첫 번째 중분류"},
    {"role": "detail_item", "text": "ㅇ 세부 내용 1"},
    {"role": "detail_item", "text": "ㅇ 세부 내용 2"},
    {"role": "chapter_title_box", "text": "Ⅱ. 두 번째 대분류"},
    {"role": "subsection", "text": "□ 두 번째 중분류"},
...
- 각 항목의 **role**은 양식 구조 분석 결과에 있는 role 중 하나를 사용
- 각 항목의 **text**는 소스 자료의 해당 내용
- marker(□, ㅇ, -, * 등)가 있는 role이면 text 앞에 marker를 포함하세요
- spacer, toc, fixed role은 사용하지 마세요 — 시스템이 자동 처리합니다

### 내용 개수
- 소스 내용이 양식보다 많아도 됩니다 — 같은 role을 반복 사용하면 시스템이 자동으로 복제합니다
- 소스 내용이 양식보다 적어도 됩니다 — 사용하지 않는 양식 슬롯은 시스템이 자동 제거합니다
- **소스 자료의 내용 개수에 맞게 자유롭게 body 항목을 생성하세요**
...
def build_content_mapping_prompt(
    ...
    for role_name, info in role_catalog.items():
        ...
        catalog_lines.append(
            f"- **{role_name}** (level {info['level']}{marker}): {info['description']}"
        )
    catalog_text = "\n".join(catalog_lines)
