...
### 양식은 패턴입니다
양식의 구조는 "이런 형식으로 작성하라"는 가이드입니다. 소스 자료의 섹션/항목 수가 양식과 다를 수 있습니다:
- **소스가 더 많으면**: 양식의 기존 문단을 채운 뒤, add_paragraph로 같은 서식(paraPrIDRef, charPrIDRef)으로 추가하세요. 내용을 생략하지 말고 전부 반영하세요
- **소스가 더 적으면**: 남는 문단을 빈 문자열로 비우거나 remove_paragraph로 제거하세요
양식의 섹션 개수에 맞추려고 내용을 축약하거나 생략하지 마세요. 소스 자료의 모든 내용을 반영하는 것이 최우선입니다.
...
row=0, col=0 이외의 좌표는 절대 사용하지 마세요.
...
## 중요
1. "(고정 텍스트, 수정 불필요)"가 아닌 모든 paragraph idx에 대해 set_paragraph_text를 생성하세요 (빠뜨리면 내용이 사라집니다)
2. 표의 value_cells에 대해 모두 set_cell을 생성하세요. 표 번호는 _tbl_idx 값을 사용하세요
3. 양식보다 소스 내용이 많으면 add_paragraph를 자유롭게 사용하세요. paraPrIDRef, charPrIDRef는 같은 level의 문단 값을 복사하세요. 내용을 생략하지 말고 전부 반영하세요
4. 양식보다 소스 내용이 적으면 남는 문단을 빈 문자열로 비우세요
5. 소스 자료의 모든 내용을 빠짐없이 반영하세요. 양식 슬롯 부족은 이유가 되지 않습니다 — add_paragraph로 확장하세요
"""
