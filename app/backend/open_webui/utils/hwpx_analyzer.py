...
- **role**: 이 문단의 의미/구조 역할. **같은 마커 종류 + 같은 level이면 반드시 같은 role** (paraPrIDRef 차이는 무시)
- **paraPrIDRef**: <hp:p>의 paraPrIDRef 속성값 (참고용 — role 분류 기준 아님)
- **charPrIDRef**: 첫 번째 <hp:run>의 charPrIDRef 속성값 (참고용 — role 분류 기준 아님)

### role 부여 규칙
role은 **마커 종류 + level(계층 깊이)**이 기준입니다. 서식 속성(paraPrIDRef, charPrIDRef)은 role 분류에 사용하지 마세요.

1. **같은 마커 종류 + 같은 level → 반드시 같은 role** (예: 모든 "□" level 3 문단 → section_header)
2. **paraPrIDRef/charPrIDRef가 달라도 마커와 level이 같으면 같은 role을 사용하세요** — 서식 미세 차이로 role을 나누지 마세요
3. **다른 마커 종류이면 다른 role** (예: ㅇ와 ➊는 다른 role)
4. **🚫 alt1, alt2, _sub, _1, _2 같은 숫자/접미사로 role 세분화 금지** — 같은 의미면 하나의 role 이름만 사용
   잘못된 예: task_title_box, task_title_box_alt1, task_title_box_alt2, ..., task_title_box_alt8 ← 금지
   올바른 예: 모두 task_title_box (하나의 이름)
5. 마커가 없는 문단은 **위치와 의미**로 판단: cover_title, cover_date, cover_org, toc, chapter_title, strategy_header, task_header 등
6. role 이름은 자유롭게 지정하되, 용도를 알 수 있는 의미있는 이름을 사용하세요
   예: "chapter_title", "section_header", "detail_item", "note", "summary_box", "toc", "cover_title", "task_header", "task_detail"
7. 특수 역할:
   - "spacer": 빈 문단 (구분용 빈 줄)
   - "fixed": 페이지 번호, 머리글/바닥글 등 수정 불필요한 레이아웃 요소

### 마커와 계층 관계
마커(번호 기호)는 양식마다 다릅니다. 고정된 계층 순서는 없습니다.
**양식 XML의 실제 구조(들여쓰기, 위치, 포함 관계)를 보고 트리 관계를 직접 파악하세요.**

판단 기준:
1. **들여쓰기가 더 깊은 문단 = 바로 위 문단의 자식** (level이 높음)
2. **같은 들여쓰기에서 마커 종류가 같으면 = 같은 level의 형제**
3. **마커 종류가 바뀌면서 들여쓰기가 깊어지면 = 새로운 하위 level**

마커 종류 예시 (순서가 아닌 분류용):
- 순번형: Ⅰ/Ⅱ/Ⅲ, 󰊱/󰊲/󰊳, ➊/➋/➌, ①/②/③, 1)/2)/3), 가./나./다.
- 기호형: □, ㅇ, -, ▪, ◈, ◇, ◆, ⇒, →
- 보충형: *, **, ***, ※

**같은 마커 종류가 다른 부모 아래에서 나올 수 있습니다.** 이 경우 위치(어떤 부모 아래인지)로 구분하세요.
...
- **role 분류 기준: 마커 종류 + level(계층 깊이)** — paraPrIDRef/charPrIDRef는 role 분류 기준이 아닙니다
- **🚫 alt1, alt2, _sub, _1, _2 같은 숫자/접미사로 role 세분화 금지** — 서식 미세 차이로 같은 의미의 role을 여러 이름으로 나누지 마세요
  잘못된 예: task_title_box, task_title_box_alt1, ..., task_title_box_alt24 ← 절대 금지
  올바른 예: 모두 task_title_box (하나의 이름으로 통일)
...
- **level 값은 실제 계층 깊이를 정확히 반영** — 들여쓰기, 마커 종류, 위치로 판단
"""
...
def parse_structure_from_llm(llm_response: str) -> dict:
    ...
    # 후처리: chapter_types를 paragraphs의 level/role 순서에서 코드로 생성
    # (AI가 만든 chapter_types는 무시하고 덮어씀)
    data["chapter_types"] = _build_chapter_types(data.get("paragraphs", []))
