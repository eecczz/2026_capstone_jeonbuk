...
def build_adaptation_plan_prompt(
    ...
    user_msg = (
        ...
        "**길이 가이드 (양식 layout 보존, 변경은 권장):**\n"
        "- adapted_title의 글자 수는 original_title과 비슷한 범위 (±20% 정도). 양식 layout 깨짐 방지용.\n"
        "- **단, source 도메인 보정은 반드시 적용**. 길이 맞추려고 source 도메인 token 교체를 생략하지 마세요.\n"
        "  예: original='2024년 업무추진 여건 및 방향', source 도메인=클라우드\n"
        "    → '2026년 클라우드 전환 여건 및 방향' (도메인 token '업무추진' → '클라우드 전환' 교체. 길이 비슷).\n"
        "    금지: '2026년 업무추진 여건 및 방향' (연도만 바꿈, 도메인 보정 누락).\n"
        "- 길이를 위해 부가 수식어를 욕심내지 말 것. 도메인 token swap이 우선.\n\n"
