...
_FILE_URL_RE = re.compile(r'/api/v1/files/([0-9a-f][0-9a-f\-]{20,50})/content')


def _fix_file_download_urls_in_output(output, events):
    """
    LLM이 tool result의 file download UUID를 응답에 복사할 때 변형하는 문제 수정.
    tool result(sources)의 정확한 UUID와 LLM 응답의 UUID를 비교하여 교정.
    """
    correct_uuids = set()

    # events(sources)에서 정확한 file UUID 수집 (non-native tool calling)
    for event in (events or []):
        for source in event.get("sources", []):
            if not source.get("tool_result"):
                continue
            for doc in source.get("document", []):
                for m in _FILE_URL_RE.finditer(str(doc)):
                    correct_uuids.add(m.group(1))

    # output의 function_call_output에서도 수집 (native tool calling)
    for item in output:
        if item.get("type") == "function_call_output":
            for part in item.get("output", []):
                for m in _FILE_URL_RE.finditer(part.get("text", "")):
                    correct_uuids.add(m.group(1))

    if not correct_uuids:
        return

    # message 아이템의 텍스트에서 UUID 교정
    for item in output:
        ...
        for part in item.get("content", []):
            ...
            for m in _FILE_URL_RE.finditer(text):
                ...
                if found_uuid in correct_uuids:
                    continue
                # 가장 유사한 정확한 UUID 찾기
                best = max(correct_uuids, key=lambda c: SequenceMatcher(None, found_uuid, c).ratio())
                ratio = SequenceMatcher(None, found_uuid, best).ratio()
                if ratio > 0.6:
                    text = text.replace(found_uuid, best)
                    changed = True
                    log.info(f"FILE_URL_FIX: Corrected corrupted UUID {found_uuid} -> {best} (similarity={ratio:.2f})")
            if changed:
                part["text"] = text
                ...
                sources_by_file[key]["metadata"].append(
                    {
                        ...
                        **({"page": chunk["page"]} if chunk.get("page") is not None else {}),
                        **({"page_label": chunk["page_label"]} if chunk.get("page_label") else {}),
                        **({"total_pages": chunk["total_pages"]} if chunk.get("total_pages") else {}),
                    }
                )
...
def get_source_context(
    ...
    for source in sources:
        for doc, meta in zip(source.get("document", []), source.get("metadata", [])):
            ...
            # JBTP patch: Add PDF page number to citation source name
            _page_label = meta.get("page_label")
            if not _page_label and meta.get("page") is not None:
                _page_label = str(int(meta["page"]) + 1)
            if _page_label and src_name:
                _total = meta.get("total_pages")
                if _total:
                    src_name = f"[p.{_page_label}/{_total}] {src_name}"
                else:
                    src_name = f"[p.{_page_label}] {src_name}"
...
async def chat_completion_tools_handler(
    ...
    def get_tools_function_calling_payload(messages, task_model_id, content):
        ...
        # Fallback: if content is a list without text parts, extract from all parts
        if not user_message and messages:
            last_user = get_last_user_message_item(messages)
            if last_user:
                msg_content = last_user.get("content", "")
                if isinstance(msg_content, list):
                    ...
                    user_message = msg_content.strip()
        ...
        # Filter out system messages — they confuse the task model
        # (e.g. SUNO system prompt being treated as lyrics)
        recent_messages = [m for m in recent_messages if m.get("role") != "system"]
        chat_history = "\n".join(
    ...
    try:
        log.info(f"TOOL_HANDLER: Calling task model={task_model_id}")
        log.info(f"TOOL_HANDLER: payload_system={repr(payload['messages'][0]['content'])[:500]}")
        log.info(f"TOOL_HANDLER: payload_user={repr(payload['messages'][1]['content'])[:500]}")
        response = await generate_chat_completion(request, form_data=payload, user=user)
        content = await get_content_from_response(response)
        log.info(f"TOOL_HANDLER: content={repr(content)[:300]}")

        if not content:
            log.warning("TOOL_HANDLER: Empty content from task model")
            return body, {}

        try:
            # Strip Harmony-style analysis/channel tokens before JSON extraction
            import re as _re
            _harmony_analysis = _re.compile(
                r"<\|channel\|>analysis<\|message\|>.*?<\|end\|>", _re.DOTALL
            )
            _harmony_final = _re.compile(
                r"<\|start\|>\s*assistant\s*<\|channel\|>final<\|message\|>"
            )
            _harmony_special = _re.compile(r"<\|[^|]*\|>")
            content = _harmony_analysis.sub("", content)
            content = _harmony_final.sub("", content)
            content = _harmony_special.sub("", content)
            ...
            log.info(f"TOOL_HANDLER: parsed result={result}")

            async def tool_call_handler(tool_call):
                nonlocal skip_files

                log.info(f"TOOL_HANDLER: tool_call={tool_call}")

                tool_function_name = tool_call.get("name", None)
                if tool_function_name not in tools:
                    # MCP tools are registered as server_id_toolname; LLM may return just toolname
                    _resolved = None
                    for _tk in tools:
                        if _tk.endswith(f"_{tool_function_name}"):
                            _resolved = _tk
                            break
                    if _resolved:
                        log.info(f"TOOL_HANDLER: resolved {tool_function_name} -> {_resolved}")
                        tool_function_name = _resolved
                    else:
                        log.warning(f"TOOL_HANDLER: tool {tool_function_name} not found in {list(tools.keys())}")
                        return body, {}
                ...
                try:
                    ...
                    _display_name = tool_function_name.split("_", 1)[-1] if "_" in tool_function_name else tool_function_name
                    if event_emitter:
                        await event_emitter(
                            {
                                "type": "status",
                                "data": {
                                    "action": "tool_call",
                                    "description": f"도구 실행 중: {_display_name}",
                                    "done": False,
                                },
                            }
                        )
                    ...
                    if tool.get("direct", False):
                        ...
                        # Inject __event_emitter__ so tools can emit streaming status/content
                        tool_function_params["__event_emitter__"] = event_emitter
                        tool_result = await tool_function(**tool_function_params)

                except Exception as e:
                    tool_result = str(e)

                # Emit status done event after tool execution
                if event_emitter:
                    await event_emitter(
                        {
                            "type": "status",
                            "data": {
                                "action": "tool_call",
                                "description": f"도구 실행 완료: {_display_name}",
                                "done": True,
                            },
                        }
                    )
                    ...
                    sources.append(
                        {
                            ...
                            "metadata": [
                                {
                                    "source": (f"{tool_name}"),
                                    "parameters": {k: v for k, v in tool_function_params.items() if not k.startswith(chr(95)*2)},
                                }
                            ],
                            "tool_result": True,
                        }
            ...
            if result.get("tool_calls"):
                ...
                # Fallback: if task model returned empty and only 1 tool, auto-call it
                # Skip auto-call for chat_context (context-only tool that must go through LLM)
                if len(tools) == 1 and list(tools.keys())[0] != "chat_context":
                    _only_tool_name = list(tools.keys())[0]
                    _only_tool_spec = tools[_only_tool_name].get("spec", {})
                    _required = _only_tool_spec.get("parameters", {}).get("required", [])
                    # Smart default params: extract conversation context
                    _default_params = {}
                    _msgs = body.get("messages", [])
                    _bad_chars = set('\\/:"*?<>|')
                    for p in _required:
                        if p == "user_id":
                            _default_params[p] = user.id if hasattr(user, "id") else ""
                        elif p == "content":
                            # Use last assistant message as document content
                            _asst = [m.get("content", "") for m in _msgs if m.get("role") == "assistant" and isinstance(m.get("content", ""), str)]
                            if _asst:
                                _default_params[p] = _asst[-1]
                            else:
                                # Fallback: last user message
                                _usr = [m.get("content", "") for m in _msgs if m.get("role") == "user" and isinstance(m.get("content", ""), str)]
                                _default_params[p] = _usr[-1] if _usr else ""
                        elif p == "file_name":
                            _usr2 = [m.get("content", "") for m in _msgs if m.get("role") == "user" and isinstance(m.get("content", ""), str)]
                            _hint = _usr2[-1][:60].strip() if _usr2 else "document"
                            _hint = "".join(c for c in _hint if c not in _bad_chars)
                            _default_params[p] = _hint[:50] if _hint else "document"
                        else:
                            # Fallback: use last user message for unknown required params (e.g. "request")
                            _usr_fb = [m.get("content", "") for m in _msgs if m.get("role") == "user" and isinstance(m.get("content", ""), str)]
                            _default_params[p] = _usr_fb[-1] if _usr_fb else ""
                    log.info(f"TOOL_HANDLER: Auto-calling single tool '{_only_tool_name}' (task model returned empty), params keys={list(_default_params.keys())}")
                    await tool_call_handler({"name": _only_tool_name, "parameters": _default_params})

        except Exception as e:
            log.error(f"TOOL_HANDLER: JSON/tool error: {e}")
            content = None

            # Fallback: JSON parsing failed but there's only 1 tool — auto-call it
            # (same logic as the successful-parse fallback above)
            if len(tools) == 1 and list(tools.keys())[0] != "chat_context":
                try:
                    _only_tool_name = list(tools.keys())[0]
                    _only_tool_spec = tools[_only_tool_name].get("spec", {})
                    ...
                    for p in _required:
                        if p == "user_id":
                            ...
                            _usr_fb = [m.get("content", "") for m in _msgs if m.get("role") == "user" and isinstance(m.get("content", ""), str)]
                            _default_params[p] = _usr_fb[-1] if _usr_fb else ""
                    log.info(f"TOOL_HANDLER: Auto-calling single tool '{_only_tool_name}' (JSON parse failed), params keys={list(_default_params.keys())}")
                    await tool_call_handler({"name": _only_tool_name, "parameters": _default_params})
                except Exception as fallback_err:
                    log.error(f"TOOL_HANDLER: Fallback auto-call also failed: {fallback_err}")
    except Exception as e:
        log.error(f"TOOL_HANDLER: generate_chat_completion error: {e}")
        content = None

    log.info(f"TOOL_HANDLER: sources={sources}")

    # When a single-tool model had its tool called, skip the main LLM call
    # so the LLM doesn't overwrite the tool's output (e.g. GarPlayer, images)
    if len(tools) == 1 and sources and list(tools.keys())[0] != "chat_context":
        _tool_doc = sources[-1].get("document", [])
        body["_skip_llm"] = True
        body["_tool_content"] = "\n".join(_tool_doc)
        log.info(f"TOOL_HANDLER: _skip_llm=True (single tool), tool emitted {len(body['_tool_content'])} chars")
...
async def chat_web_search_handler(
    ...
    try:
        ...
        try:
            ...
            # queries = queries[:2]  # Hard cap removed
        except Exception as e:
            queries = [response]
    ...
    # Fallback to user message if queries are empty
    if len(queries) == 0 or (len(queries) == 1 and queries[0].strip() == ""):
        queries = [user_message]
    ...
    await event_emitter(
        {
            ...
            "data": {
                "action": "web_search",
                "description": f"Searching ({len(queries)} queries)...",
                "queries": queries,
                "done": False,
            },
        }
            ...
            await event_emitter(
                {
                    ...
                    "data": {
                        "action": "web_search",
                        "description": f"Searched {{{{count}}}} sites ({len(queries)} queries)",
                        "urls": results["filenames"],
...
async def chat_image_generation_handler(
    ...
    # Block image generation if model explicitly disables it
    _model_id = form_data.get("model", "")
    try:
        from open_webui.models.models import Models as _Models
        _m = _Models.get_model_by_id(_model_id)
        if _m and hasattr(_m, "meta"):
            _m_meta = _m.meta.model_dump() if hasattr(_m.meta, "model_dump") else (_m.meta if isinstance(_m.meta, dict) else {})
            if _m_meta.get("capabilities", {}).get("image_generation") is False:
                log.info(f"IMAGE_GEN_BLOCKED: model {_model_id} disables image_generation")
                return form_data
    except Exception as _e:
        log.warning(f"IMAGE_GEN_CHECK: could not verify model caps: {_e}")
...
async def chat_completion_files_handler(
    ...
    if files := body.get("metadata", {}).get("files", None):
        ...
        # ── Smart RAG: 파일 크기 기반 자동 full_context 판단 ──
        # 10MB 미만 파일은 RAG 청크 대신 전체 내용을 모델에 전달
        _SMART_RAG_THRESHOLD = 10 * 1024 * 1024  # 10MB
        _rag_full_raw = request.app.state.config.RAG_FULL_CONTEXT
        _rag_full = _rag_full_raw if isinstance(_rag_full_raw, bool) else str(_rag_full_raw).lower() == "true"
        smart_full_context = all_full_context or _rag_full

        if not smart_full_context:
            try:
                from open_webui.models.files import Files as _SmartFiles
                _TOOL_FILE_EXTS = {".hwp", ".hwpx"}
                # HWP/HWPX 파일이 하나라도 있으면 도구 기반 워크플로우 → Smart RAG 스킵
                _has_tool_file = any(
                    _f.get("name", _f.get("filename", "")).lower().endswith(tuple(_TOOL_FILE_EXTS))
                    for _f in files if _f.get("type") == "file"
                )
                _total_size = 0
                if _has_tool_file:
                    log.info("SMART_RAG: hwp/hwpx 파일 포함 → 도구 워크플로우, Smart RAG 스킵")
                else:
                    for _f in files:
                        if _f.get("type") == "file" and _f.get("id"):
                            _fm = _SmartFiles.get_file_metadata_by_id(_f["id"])
                            if _fm and isinstance(getattr(_fm, "meta", None), dict):
                                _total_size += _fm.meta.get("size", 0)
                if 0 < _total_size < _SMART_RAG_THRESHOLD:
                    smart_full_context = True
                    log.warning(f"SMART_RAG: auto full_context=True (total_size={_total_size}B < {_SMART_RAG_THRESHOLD}B)")
                elif _total_size >= _SMART_RAG_THRESHOLD:
                    log.warning(f"SMART_RAG: full_context=False (total_size={_total_size}B >= {_SMART_RAG_THRESHOLD}B)")
                    # 10MB 초과 대용량 파일: GPT/Gemini 사용 안내
                    await __event_emitter__(
                        {
                            "type": "status",
                            "data": {
                                "description": "📄 대용량 파일입니다. 전체 내용 분석이 필요하면 ChatGPT 또는 Gemini (1M 토큰) 모델을 사용해주세요.",
                                "done": True,
                            },
                        }
                    )
            except Exception as _e:
                log.warning(f"SMART_RAG: error: {_e}")
        # ── End Smart RAG ──
        ...
        try:
            ...
            sources = await get_sources_from_items(
                ...
                full_context=smart_full_context,
                user=user,
            )
        except Exception as e:
            log.exception(e)
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    try:
        ...
        log.error(f"INLET_FILTER_ERROR: {_tb.format_exc()}")
        raise Exception(f"{e}")
    ...
    if features:
        ...
        if "web_search" in features and features["web_search"]:
            ...
            if metadata.get("params", {}).get("function_calling") != "native":
                ...
                # Disable thinking tokens for web search to reduce latency (Qwen3.5)
                model_id = form_data.get("model", "")
                if "qwen" in model_id.lower() and "3.5" in model_id:
    ...
    # Auto-fetch URLs from short messages (link-only prompts)
    if urls:
        if not files:
            files = []
        files.extend([{"type": "url", "url": url, "name": url} for url in urls])
    ...
    if not payload_tools:
        ...
        if tool_ids:
            for tool_id in tool_ids:
                if tool_id.startswith("server:mcp:"):
                    _mcp_server_id_for_error = tool_id  # safe fallback for except block
                    try:
                        _mcp_rest = tool_id[len("server:mcp:"):]
                        # Parse "server_id:tool_name" or just "server_id"
                        if ":" in _mcp_rest:
                            server_id, _mcp_tool_filter = _mcp_rest.split(":", 1)
                        else:
                            server_id = _mcp_rest
                            _mcp_tool_filter = None
                        log.info(f"MCP tool lookup: server_id={server_id}, tool_filter={_mcp_tool_filter}")
                        ...
                        _mcp_hdrs = headers if headers else None
                        _mcp_connected = False
                        for _mcp_attempt in range(3):
                            try:
                                ...
                                _mcp_connected = True
                                break
                            except BaseException as _mcp_err:
                                log.warning(f"MCP connect attempt {_mcp_attempt+1}/3 failed for {server_id}: {_mcp_err}")
                                if _mcp_attempt < 2:
                                    import asyncio as _aio
                                    await _aio.sleep(0.5)
                        if not _mcp_connected:
                            log.error(f"MCP connection failed after 3 attempts for {server_id}")
                            if event_emitter:
                                await event_emitter(
                                    {
                                        ...
                                        "data": {
                                            "error": {
                                                "content": f"Failed to connect to MCP server '{server_id}' after 3 attempts"
                                            }
                                        },
                                    }
                                )
                        ...
                        for tool_spec in tool_specs:
                            # Filter by specific tool name if specified in toolId (server:mcp:server_id:tool_name)
                            if _mcp_tool_filter and tool_spec["name"] != _mcp_tool_filter:
                                continue
                        ...
                        log.error(f"MCP connection error for {_mcp_server_id_for_error}: {e}")
                        import traceback
                        log.error(traceback.format_exc())
                        if event_emitter:
                            _err_id = server_id if 'server_id' in dir() else _mcp_server_id_for_error
                            await event_emitter(
                                {
                                    "type": "chat:message:error",
                                    "data": {
                                        "error": {
                                            "content": f"Failed to connect to MCP server '{_err_id}'"
                                        }
                                    },
                                }
                            )
            ...
            # Filter out private helper methods (_xxx) from tool classes to prevent
            # them from polluting the tools list (e.g. SUNO _get/_post/_poll etc.)
            tools_dict = {k: v for k, v in tools_dict.items() if not k.startswith('_')}

            if mcp_tools_dict:
                tools_dict = {**tools_dict, **mcp_tools_dict}
                log.info(f"MCP_DEBUG: mcp_tools={list(mcp_tools_dict.keys())}, total_tools={list(tools_dict.keys())}")
            else:
                log.info(f"MCP_DEBUG: mcp_tools_dict is EMPTY, tools_dict keys={list(tools_dict.keys())}")
        ...
        if tools_dict:
            # For single-tool models, force non-native function calling so the
            # middleware tool handler auto-calls the tool and sets _skip_llm,
            # preventing the LLM from overwriting rich tool output (e.g. GarPlayer images)
            _use_native = (
                metadata.get("params", {}).get("function_calling") == "native"
                and (len(tools_dict) > 1 or (len(tools_dict) == 1 and "chat_context" in tools_dict))
            )
            if _use_native:
    ...
    # Upstage OCR 모델은 파일을 자체 처리하므로 RAG 파일 처리 스킵
    _skip_file_processing = form_data.get("model", "").startswith("upstage_document_ocr")
    if file_context_enabled and not _skip_file_processing:
        try:
    ...
    # JBTP: Enrich sources missing page metadata from Qdrant vector DB
    # When type="file" full-context reads from DB, page metadata is lost.
    # This post-processing step fetches page-level chunks from Qdrant to restore it.
    try:
        from open_webui.retrieval.utils import get_all_items_from_collections as _get_all_items
        for _src in sources:
            _metas = _src.get("metadata") or []
            if not _metas:
                continue
            # Skip sources that already have page metadata
            if any(_m.get("page") is not None or _m.get("page_label") for _m in _metas):
                continue
            # Find file_id from source info or metadata
            _file_id = (_src.get("source") or {}).get("id")
            if not _file_id and _metas:
                _file_id = _metas[0].get("file_id")
            if not _file_id:
                continue
            _qr = _get_all_items([f"file-{_file_id}"])
            if _qr and _qr.get("documents") and _qr["documents"][0]:
                _src["document"] = _qr["documents"][0]
                _src["metadata"] = _qr["metadatas"][0]
                log.debug(f"JBTP page enrichment: file {_file_id} -> {len(_qr['documents'][0])} chunks with page metadata")
    except Exception as _enrich_err:
        log.warning(f"JBTP page enrichment error: {_enrich_err}")

    # JBTP patch: Split collection sources by file and add page numbers
    _expanded_sources = []
    for _src in sources:
        _src_info = _src.get("source", {})
        _src_type = _src_info.get("type", "")
        _metas = _src.get("metadata") or []
        _docs = _src.get("document") or []

        # For collections, split by individual file (name in metadata)
        if _src_type == "collection" and _metas:
            _by_file = {}
            for _idx, _m in enumerate(_metas):
                _fname = _m.get("name") or _m.get("source") or _src_info.get("name", "")
                if _fname not in _by_file:
                    _by_file[_fname] = {"docs": [], "metas": [], "total": None}
                _by_file[_fname]["docs"].append(_docs[_idx] if _idx < len(_docs) else "")
                _by_file[_fname]["metas"].append(_m)
                if _m.get("total_pages"):
                    _by_file[_fname]["total"] = _m["total_pages"]

            for _fname, _fdata in _by_file.items():
                _pages = set()
                for _m in _fdata["metas"]:
                    _pl = _m.get("page_label")
                    if not _pl and _m.get("page") is not None:
                        _pl = str(int(_m["page"]) + 1)
                    if _pl:
                        try:
                            _pages.add(int(_pl))
                ...
                if _pages:
                    ...
                    for _p in _sorted[1:]:
                        if _p == _end + 1:
                            ...
                            _ranges.append(str(_start) if _start == _end else str(_start) + "-" + str(_end))
                            _start = _end = _p
                    _ranges.append(str(_start) if _start == _end else str(_start) + "-" + str(_end))
                    if len(_ranges) > 5:
                        _page_str = ", ".join(_ranges[:5]) + "..."
                    else:
                        _page_str = ", ".join(_ranges)
                    _total = _fdata["total"]
                    if _total:
                        _display_name = "[p." + _page_str + "/" + str(_total) + "] " + _fname
                    else:
                        _display_name = "[p." + _page_str + "] " + _fname

                _expanded_sources.append({
                    "source": {**_src_info, "name": _display_name},
                    "document": _fdata["docs"],
                    "metadata": _fdata["metas"],
                })
        else:
            # Non-collection sources: add page info to name directly
            _pages = set()
            _total = None
            for _m in _metas:
                _pl = _m.get("page_label")
                ...
                if _pl:
                    try:
                        ...
                        pass
                if _m.get("total_pages"):
                    _total = _m["total_pages"]

            _name = _src_info.get("name", "")
            if _pages and _name:
                _sorted = sorted(_pages)
                _ranges = []
                _start = _sorted[0]
                ...
                if len(_ranges) > 5:
                    ...
                    _page_str = ", ".join(_ranges)
                if _total:
                    _src_info["name"] = "[p." + _page_str + "/" + str(_total) + "] " + _name
                else:
                    _src_info["name"] = "[p." + _page_str + "] " + _name

            _expanded_sources.append(_src)

    sources = _expanded_sources
    ...
    if event_emitter:
        try:
            ...
            if choices and choices[0].get("message", {}).get("content"):
                ...
                # Fix LLM-corrupted file download URLs (non-streaming)
                try:
                    correct_uuids = set()
                    for ev in (events or []):
                        for src in ev.get("sources", []):
                            if not src.get("tool_result"):
                                continue
                            for doc in src.get("document", []):
                                for _m in _FILE_URL_RE.finditer(str(doc)):
                                    correct_uuids.add(_m.group(1))
                    if correct_uuids and content:
                        for _m in _FILE_URL_RE.finditer(content):
                            fu = _m.group(1)
                            if fu not in correct_uuids:
                                best = max(correct_uuids, key=lambda c: SequenceMatcher(None, fu, c).ratio())
                                if SequenceMatcher(None, fu, best).ratio() > 0.6:
                                    content = content.replace(fu, best)
                                    log.info(f"FILE_URL_FIX(non-stream): {fu} -> {best}")
                        response_data["choices"][0]["message"]["content"] = content
                except Exception as e:
                    log.warning(f"FILE_URL_FIX(non-stream): error: {e}")
...
async def streaming_chat_response_handler(response, ctx):
    ...
    if event_emitter and event_caller:
        ...
        model_id = form_data.get("model", "")
        provider = model.get("owned_by") or model.get("provider")
        ...
        async def response_handler(response, events):
            ...
            try:
                ...
                usage_recorded = await stream_body_handler(response, form_data)
                if not usage_recorded and metadata.get("user_id"):
                    encoding_name = request.app.state.config.TIKTOKEN_ENCODING_NAME
                    prompt_tokens = estimate_tokens_for_messages(
                        form_data.get("messages", []), encoding_name
                    )
                    completion_text = ""
                    for _item in output:
                        if _item.get("type") == "message":
                            for _part in _item.get("content", []):
                                if "text" in _part:
                                    completion_text += _part.get("text", "")
                    completion_text = completion_text.strip()
                    completion_tokens = estimate_tokens_for_text(
                        completion_text, encoding_name
                    )
                    total_tokens = sum_token_counts(prompt_tokens, completion_tokens)
                ...
                # Fix LLM-corrupted file download URLs before finalization
                try:
                    _fix_file_download_urls_in_output(output, events)
                except Exception as e:
                    log.warning(f"FILE_URL_FIX: error during URL correction: {e}")
        ...
        async def stream_wrapper(original_generator, events):
            ...
            async for line in original_generator:
                ...
                # Skip empty lines - pass through as-is
                stripped = line.strip() if line else ""
                if not stripped:
                    continue

                # Non-SSE data lines - pass through as-is
                ...
                # Parse JSON and apply stream filters
                try:
                ...
                try:
                    ...
                    # If filter fails, pass original data through
