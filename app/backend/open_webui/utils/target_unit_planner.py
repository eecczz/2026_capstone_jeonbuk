...
def _suggest_body_unit_type(
    paragraphs: list,
    body_indices: list,
    chapter_title_positions: list,
    unit_observations: list | None,
) -> str:
    """
    Body region의 unit_type을 suggestion. AI가 최종 확정.
    12.0 unit_observations를 참고하되, 확정하지 않음.
    """
    # Check if 12.0 says chapter is strong
    if unit_observations:
        strong_units = {
            u["unit_type"] for u in unit_observations
            if u.get("observed_role") == "strong_candidate"
        }
        if "chapter" in strong_units and "shallow_block" not in strong_units and "table" not in strong_units:
            return "chapter"
        if "shallow_block" in strong_units or "table" in strong_units:
            if "chapter" not in strong_units:
                return "shallow_block"

    # Fallback: count chapter titles in body
    body_set = set(body_indices)
    chapters_in_body = [pos for pos in chapter_title_positions if pos in body_set]
    if len(chapters_in_body) >= 2:
        return "chapter"

    # Default: undetermined (let AI decide)
    return "body_undetermined"
