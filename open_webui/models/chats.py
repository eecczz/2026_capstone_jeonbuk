    ...
    def get_chats_by_user_id_and_search_text(
        ...
        with get_db_context(db) as db:
            ...
            if dialect_name == "sqlite":
                ...
                # Optimized: use chat_message table + pg_trgm GIN index
                # instead of json_array_elements on 24GB chat JSON column
                # Performance: 15s -> 188ms (80x improvement)
                postgres_content_sql = """
                EXISTS (
                    SELECT 1 FROM chat_message cm
                    WHERE cm.chat_id = chat.id
                      AND (cm.content#>>'{}') ILIKE '%' || :content_key || '%'
                )
                """
