...
async def search_chats(
    ...
    Optimized: uses chat_message table with pg_trgm GIN indexes instead of JSON parsing.
    ...
    try:
        ...
        with get_db() as db:
                    ...
                    WHERE c.user_id = :user_id AND c.archived = false
                      AND EXISTS (
                        ...
                        WHERE cm.chat_id = c.id
                          AND (cm.content#>>'{}'::text[]) ILIKE :pattern
                      )
                    ORDER BY c.updated_at DESC
                    LIMIT :lim
                ),
            ...
            for row in rows:
                chat_id, title, updated_at, snippet = row

                if __chat_id__ and chat_id == __chat_id__:
                    continue
                if start_timestamp and updated_at < start_timestamp:
                    continue
                if end_timestamp and updated_at > end_timestamp:
                    continue
                ...
                if snippet:
                    ...
                    lower_query = search_text.lower()
                    idx = lower_snippet.find(lower_query)
                    if idx >= 0:
                        s = max(0, idx - 50)
                        e = min(len(snippet), idx + len(search_text) + 100)
                        display_snippet = (
                            ("..." if s > 0 else "")
                            + snippet[s:e]
                            + ("..." if e < len(snippet) else "")
                        )
                    else:
                        display_snippet = snippet[:150]
                else:
                    display_snippet = f"Title match: {title}"
        ...
        if collection_names:
            ...
            if query_results and "documents" in query_results:
                ...
                for idx, doc in enumerate(documents):
                    ...
                    # Page metadata for Citation Panel
                    meta = metadatas[idx]
                    if meta.get("page") is not None:
                        chunk_info["page"] = meta["page"]
                    if meta.get("page_label"):
                        chunk_info["page_label"] = meta["page_label"]
                    if meta.get("total_pages"):
                        chunk_info["total_pages"] = meta["total_pages"]
