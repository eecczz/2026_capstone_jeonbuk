        ...
        try:
            ...
            if form_data.content:
                ...
                if result is not None and len(result.ids[0]) > 0:
                    ...
                    # JBTP patch: Re-load from file storage to preserve page metadata
                    _file_path = file.path
                    if _file_path:
                        try:
                            _file_path = Storage.get_file(_file_path)
                            _loader = Loader(
                            ...
                            docs = _loader.load(
                                file.filename, file.meta.get("content_type"), _file_path
                            )
                            docs = [
                                Document(
                            ...
                            log.warning(f"Failed to re-load file {file.id} for page metadata: {_e}")
                            docs = [
                                Document(
...
async def process_web_search(
    ...
    try:
        ...
        if request.app.state.config.BYPASS_WEB_SEARCH_EMBEDDING_AND_RETRIEVAL:
            # Truncate each document to limit total context tokens
            # 10000 chars ≈ 2500 tokens; keeps quality while preventing OOM
            MAX_DOC_CHARS = 10000
            return {
                "status": True,
                "collection_name": None,
